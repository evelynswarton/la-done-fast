...
Matrix::Matrix(unsigned numRows, unsigned numCols, vector<double> values) {
  if ((numRows * numCols) != values.size()) {
    cout << "ERROR: called constructor that accepts vector: vector passed was "
            "of incorrect length"
         << endl;
...
// Subscript operator
vector<double> Matrix::operator[](unsigned i) { return m_matrix[i]; }
...
void Matrix::swap_rows(unsigned i, unsigned j) {
  vector<double> temp = this->m_matrix[i];
  this->m_matrix[j] = this->m_matrix[i];
  this->m_matrix[i] = temp;
}

void Matrix::swap_cols(unsigned i, unsigned j) {
  for (int k = 0; k < this->m_numRows; k++) {
    double temp = this->m_matrix[k][i];
    this->m_matrix[k][i] = this->m_matrix[k][j];
    this->m_matrix[k][j] = temp;
  }
}
